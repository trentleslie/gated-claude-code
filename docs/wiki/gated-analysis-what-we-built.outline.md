...
- The architecture is a reusable template rather than a one-off deployment. The profiler, gate, and bridge are dataset-agnostic; a different tabular backend is accommodated by pointing the executor at its directory and regenerating the dictionary, and an additional instance on an identical dataset requires only the provisioning scripts and a copy of the already-audited dictionary, with no re-profiling.
...
The male excess of approximately four mg/dL is consistent with the direction and magnitude reported in the literature. A follow-up analysis confirmed the finding is robust to longitudinal weighting: averaging by draw rather than by client shifted each mean by no more than 0.2 mg/dL, and the shift was near-identical across sexes, indicating that testing frequency does not covary with glucose differently between groups. Every validation run released a result and logged a corresponding append-only audit entry, each attributable to a specific submitted script by content hash.

## Reproducibility across a fleet

The architecture was reproduced on two further virtual machines beyond the reference instance, each serving the identical Arivale snapshot, and each verified end-to-end to the same released result. Because the dictionary and synthetic samples are derived from the data rather than from the machine, they were copied to each new instance rather than regenerated; the provisioning is otherwise scripted, so an additional box reaches a working, verified state in minutes. One operational hazard recurred on every machine and warrants explicit mention: the data snapshot mounts world-readable by default, which would expose raw records to the model's user during the interval before it is locked. A single command, run immediately after each mount, restores the data-owning user's exclusive read access and confirms the model's user is denied. On this fleet no agent was authenticated during any exposure interval, so no raw record was read, but the safeguard is now a standard post-mount step rather than a remembered chore.
